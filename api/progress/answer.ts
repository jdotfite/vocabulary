import { computeEloUpdate, computeSrsInterval } from "../_lib/adaptive.js";
...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    // Look up word + user ability in parallel
    const [rawWords, rawUsers] = await Promise.all([
      sql`SELECT id, difficulty_score FROM words WHERE word = ${word}`,
      sql`SELECT ability_score FROM users WHERE id = ${userId}::uuid`,
    ]);

    const wordRows = rawWords as unknown as WordRow[];
    const userRows = rawUsers as unknown as UserRow[];
    const wordRow = wordRows[0];
    if (!wordRow) {
      return new Response("Word not found", { status: 404 });
    }

    const userAbility = userRows[0]?.ability_score ?? 50;
    const wordDifficulty = wordRow.difficulty_score ?? 50;

    // Compute Elo ability update
    const newAbility = computeEloUpdate(userAbility, wordDifficulty, isCorrect);

    // Get current streak for SRS calculation
    ...
    // Update SRS fields, ability score, and log ability change (fire in parallel)
    await Promise.all([
      sql`
      ...
      `,
      sql`
        UPDATE users SET ability_score = ${newAbility}
        WHERE id = ${userId}::uuid
      `,
      sql`
        INSERT INTO ability_log (user_id, old_score, new_score, word_id, is_correct)
        VALUES (${userId}::uuid, ${userAbility}, ${newAbility}, ${wordRow.id}::uuid, ${isCorrect})
      `,
    ]);
